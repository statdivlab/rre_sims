...
minimum_subset_distance <- new_method("min_sub_dist", "Minimum Subset Distance",
                                      method = function(model, draw) {
                                        result <- rre::minimum_subset_distance(draw,
                                                                               lambda_vec = seq(0,60,by = 10),
                                                                               partitions = 10, 
                                                                               c_seq_len = 25, 
                                                                               multiplier = 10)
                                        return(result)
...
my_sim <- new_simulation(name = "expand_test",
  ...
  generate_model(seed = 200103,
                 make_model = FCT_list,
                 ccc =  as.list(c(2000, 500, 1000)),
                 eta = list(c(0.1, 0.001),
                            c(0.1,0.00001)),
                 replicates = as.list(c(14, 10, 6)),
                 vary_along = c("eta", "ccc", "replicates")) %>%
...
write_csv(ev_with_model_params,
          paste("my_df_ccc_2000_1000_500_r_14_10_6_nsim_", input_nsim, "_ncores_", input_ncores, ".csv", sep = ""))

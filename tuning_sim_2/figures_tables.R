...
thesis_method_comparison <- function(sim_data,
  ...
  gg <- ggplot(sim_data, aes(x = alpha_delta_r, 
    ...
    # We need to change these two lines since we don't have all the methods:
...
result <- readr::read_csv("tuning_sim_2.csv")

thesis_method_comparison(result, output_name = "./method_sim_2.pdf")

# Information contained in the table for the secondary simulation:
result %>%

...
% \newcommand{\kmax}[1]{k_{\text{max}(#1)}}
% \newcommand{\kmax}[jj]{k_{\text{max}#1}}

\NewDocumentCommand{\kmax}{o}{%
    {%
        \IfNoValueTF {#1}{
          k_{\text{max}}
        }{
...
\author{ALEX PAYNTER$^\ast$, AMY WILLIS\\[4pt]
% Author addresses
...
\end{equation}
where $\widehat{\eta}_{\lambda}$ functions as a nuisance parameter.  $\eta$ is critical to estimation but we are largely concerned with estimating $C$ in the species problem.

We make extensive use of root-mean-square error as the primary criterion for evaluating the performance of estimators in simulations.  While this is ubiquitously used we define it here to avoid confusion.  Let $n_{\text{sim}}$ be the number of simulations completed for one choice of $(C, \eta, r)$, and $\widehat{C}_{\lambda \langle s \rangle}$ be the solution of Equation \ref{eq:ccc_hat_lambda} obtained in simulation $s$ with  $\lambda$.  Then we define
\begin{equation}
...
We simulated using $C = 1000$, and $r \in \{6, 10, 14\}$ to reflect the reality that few microbiome-focused labs are currently producing a large number of biological replicates (e.g. $r = 1000$ is unrealistic).  The abundance distribution of a population is characterized by $\eta$, so choosing appropriate simulation values of $\eta$ is crucial in targeting our approach to microbial data.   We used two microbial datasets as motivating examples.  \citet{walsh2014restricted} provide a dataset from soil in a Swiss apple orchard and we found $\eta = (\alpha, \delta) = (10^{-1}, 10^{-1})$ produced a frequency count table with a similar structure to their data.  The second example from \citet{tromas_2017} is a sample of cyanobacteria from Lake Champlain, and we found $\eta = (10^{-2}, 10^{-5})$ mimicked the structure of their data well.  We note that is possible that the gamma-Poisson model is not the best fit for these examples, our goal is only to choose $\eta$ in an appropriate range given the model.  Two nearby $\eta$ values are included, $\eta = \left( 10^{-1}, 10^{-3} \right)$ and $\eta = \left( 10^{-1}, 10^{-5}\right)$.  These add variety to our simulations in this section and context for our simulations in Section \ref{sec:tuning_simulation_2}.  Under each combination of $\eta$ and $r$ 100 simulations were completed.

In Table \ref{tab:eta_intuition} we provide a few summary measures for a single frequency count table generated under each choice of $\eta$.  This characterizes the $\eta$ values by the proportion of unobserved, rare or abundant species we would expect to see.  We make this information available here for the interested reader but delay our discussion of the differences until Section \ref{sec:tuning_simulations} where they strongly influence our results.

% latex table generated in R 3.5.1 by xtable 1.8-2 package
% Thu May 23 22:11:47 2019
\begin{table}[t]
\tblcaption{Summaries of the proportion of species which are unobserved, observed once (singletons), rarely observed and abundantly observed are calculated for each choice of $\eta$.    %These allow intuition on the probability scale and frequency count scale.  For instance, the proportion of species unobserved is $p_{\eta}(0)$ also equivalent to $\mathbb{E}(f_k)/C$ (see Section \ref{sec:tuning_proposals}).
We also estimate the maximum abundance $k_{\text{max}}$, which is a random variable indicating the number of times the most abundant species was observed.  In this table only $\mathbb{E}(k_{\text{max}})$ depends on $C$, and  $C = 1000$ is used here.
...
\begin{tabular}{rcccc}
 ...
 $\eta = $ & ($10^{-1}$, $10^{-1}$) & ($10^{-2}$, $10^{-5}$) & ($10^{-1}$, $10^{-3}$) & ($10^{-1}$, $10^{-5}$) \\
  ...
  Expected max abundance ($\mathbb{E}\left[k_{\text{max}}\right]$) & $4.1*10^{1}$ & $2.0*10^{5}$ & $4.0*10^{3}$ & $3.9*10^{5}$ \\
   \hline
\end{tabular}
\normalsize
\end{table}
One final setting is necessary for these simulations: a set of $\lambda$ values to test over.  Our primary concern is to ensure the set covers the optimal value, i.e. $\lambda$ which produces the lowest RMSE$(\widehat{C}_{\lambda})$.  While we do not have a theoretical way to determine the ideal $\lambda$ set, we can use the known behavior of the penalty function to reassure ourselves that the set was sufficiently large after the simulation is complete.  As $\lambda$ increases, $\widehat{C}_{\lambda}$ decreases.  If $\widehat{C}_{\lambda_0}$ is  below the true $C$, then for $\lambda > \lambda_0$ it will only stray farther from the true value, and the RMSE will increase.  Then we can rest assured that the set is large enough if most or all simulations had a $\widehat{C}_{\lambda}$ below the truth for the largest $\lambda$ tested.  This is clearly verifiable in the cases presented in Figures \ref{fig:fixed_lambda} and \ref{fig:fixed_lambda_2}. Simulating using $\lambda \in \{0, 5, \dots , 120\}$ was sufficient for all simulations in this section.
\vspace{30pt}
...
We depart from our partitioning scheme to explore another desirable property of an estimator.  An ideal estimator would produce parameter estimates $\widehat{C}$, $\widehat{\eta}$ which fit well with the frequency counts in our sample.  Let us momentarily consider a single frequency count table.  Using Equation \ref{eq:likelihood} we can find the expected frequency counts $f_k$ given $C$ and $\eta$,
\begin{equation}
...
We have proposed four tuning methods motivated by properties which would be desirable in an estimator of $C$.  However, we have not provided any theoretical guarantees that they are effective, let alone worth the additional computation time compared to Method 0.  In this section we propose to discover whether this is the case by comparing the RMSE of estimates using each method in simulations.
...
Universally superior RMSE is the most desirable property for a method, but if we recall Table \ref{tab:eta_intuition} we can characterize the performance of the methods based on the simulation parameters as well.  For example, $\eta = (10^{-2},10^{-5})$ has more abundant species and larger expected maximum abundance $\mathbb{E}(k_{\text{max}})$, but relatively few rare species.  The RMSE for Method 2 under this $\eta$ is 14-24\% smaller than Method 0, and for Method 4 the RMSE is slightly better for 2 of the 3 $r$ values.  Conversely, $\eta = (10^{-1}, 10^{-1})$ has more rare species and fewer abundant species and it is only Method 3 which has consistently improved performance.
...
%In this figure we can see that Method 3 improves the RMSE on $\eta = (10^{-1}, 10^{-5})$ primarily by lowering the estimates in a few simulations where $\widehat{C}_{[0]}$ is inflated (4000-6500).
...
% meh:
%\textcolor{red}{Find a home for me:
%However, we also point out that methods which deal with replicate frequency count tables have an inherent use.  While it is possible to create a single frequency count table for multiple samples with labeled abundance information $\{X_i\}$, many data sets are published with only the frequency counts $\{f_k\}$.  Analyzing two data sets from the same population which are stored as frequency counts is not possible with current methods, as there is no way to aggregate those into a single frequency count table.}

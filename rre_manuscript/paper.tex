...
In this paper we consider the penalised maximum likelihood approach of \citet{wang_2005} and investigate the open problem of selecting the penalization parameter. Because the true species richness $C$ is never observed for any sample, common approaches to tuning parameter selection (e.g., cross validation) cannot be readily applied in our case. We propose utilizing biological replicates to aid tuning, and so we extend the proposal of \citet{wang_2005} to the analysis of independent but homogeneous ecosystems.
...
Many mixing distributions have been proposed, including  \cite{bulmer_1974,ord_1986,norris_1998} (see \cite{bunge_1993} for a comprehensive review). While our proposal may be easily generalized to other mixing distributions, in this paper we consider the Gamma  distribution for $\Lambda$, a proposal first considered by \citet{fisher_1943}.
...
\end{align}
i.e., the likelihood is the product of a binomial probability mass function ($c \sim \text{Binomial}\left(C, 1- p_{\eta}(0)\right)$), and a multinomial probability mass function ($(f_1, f_2, \ldots)|c \sim \text{Multinomial}\left(c,  \frac{(p_{\eta}(1), p_{\eta}(2), \ldots)}{1-p_{\eta}(0)}\right)$).
...
\citet{sanathanan_1977}'s conditional approach involves first maximising  $L_c(\eta)$ to obtain a conditional abundance estimate $\widehat{\eta}_c$, then maximising $L_b(C, \widehat{\eta}_c)$ over $C$.
